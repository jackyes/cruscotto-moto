...
import { meanOver } from './lean-sim.mjs';
...
/* Curva tenuta a 30° con vibrazione, simulazione realistica (tests/lean-sim.mjs:
   orologio simulato e fix GPS a 1 Hz che riancora la velocità, come nell'app).
   Il rumore è UNIFORME fra −A e +A, con valore efficace A/√3: "±0,3 g" sono
   0,17 g RMS. Media su 5 semi, misura da 5 a 15 s; soglie = misurato + ~35%.
   Una versione precedente non riancorava la velocità al GPS e misurava circa
   il doppio: la velocità fusa derivava integrando il rumore. */
const RMS = rms => rms * Math.sqrt(3);   // semi-ampiezza del rumore uniforme con quella RMS

for (const [label, amp, max] of [
  ['±0,3 g uniforme (0,17 g RMS)', 0.3, 1.4],        // misurato 1,0°
  ['±0,6 g uniforme (0,35 g RMS)', 0.6, 2.7],        // misurato 2,0°
  ['0,3 g RMS', RMS(0.3), 2.1],                      // misurato 1,6°
  ['0,6 g RMS', RMS(0.6), 5.4],                      // misurato 4,2°
]) {
  test('sim: curva tenuta 30° con vibrazione ' + label + ' — errore medio sotto ' + max + '°', () => {
    const e = meanOver({ deg: 30, amp });
    assert.ok(e < max, 'errore medio su 5 semi ' + e.toFixed(2) + '°');
  });
}

test('sim: senza vibrazione né rumore l\'errore resta sotto mezzo grado', () => {
  const e = meanOver({ deg: 30, amp: 0, gA: 0 });
  assert.ok(e < 0.5, 'errore medio ' + e.toFixed(2) + '°');
});

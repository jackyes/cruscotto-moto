...
4. **Start Log** per registrare, **Stop Log** per fermare, **Export CSV** per scaricare.
   Con **Impostazioni → Avvia il log da solo** (spenta di default) il log parte da sé dopo 10 s sopra 20 km/h, con la velocità GPS fresca, mai in Demo né a GPS perso: dimenticare Start non costa più il giro. Serve l'app a schermo, come per il GPS. Non si ferma da solo (un semaforo o un rifornimento spezzerebbero il giro): da fermi per 10 minuti propone lo Stop. Dopo uno Stop premuto in marcia non riparte; si riarma dopo 2 minuti da fermi.
...
- **Tile in cache**: le tile OpenStreetMap già viste restano disponibili senza rete, fino a circa **4000** (~60-100 MB). Quando si supera il tetto si scartano quelle non usate da più tempo, non le prime scaricate: le tile di casa restano. Le tile si chiedono a `tile.openstreetmap.org` (senza i vecchi sottodomini a/b/c) e in CORS, così nella quota del browser pesano quanto sono davvero. Le tile salvate dalle versioni precedenti vengono riscaricate alla prima occasione con rete.
...
- **HUD in fullscreen** (in basso a sinistra, lontano dai pulsanti di zoom; in orizzontale a destra della colonna dei pulsanti): a mappa intera il cruscotto *e l'header* spariscono, quindi quello che serve in marcia viene ripetuto sopra la mappa con lo stesso linguaggio del gauge — arco da -60° a +60° riempito dal centro verso il lato della piega (verde a sinistra, ciano a destra) con la fascia rossa 50-60°, frecce del verso, pallini e valori dei massimi di piega della sessione, l'anello della piega cinematica (velocità × imbardata GPS: quando sta sull'arco la misura è solida, quando se ne stacca c'è qualcosa che non torna), piega attuale colorata come sul cruscotto e velocità in km/h col cartello del limite e i km di sessione. Oltre il limite (+3 km/h, come il badge del cruscotto) il cartello si riempie di rosso e la velocità diventa una pastiglia rossa; con affidabilità della piega bassa arco e numero si attenuano, e tre tacche in alto dicono se l'affidabilità è alta, media o bassa. In alto l'HUD porta anche ciò che spariva con l'header: `● REC` che pulsa quando stai registrando, lo stato del GPS col pallino e la precisione in metri, il beccheggio e l'ora. Fuori dal fullscreen non c'è e non viene aggiornato. Senza calibrazione la piega resta `--`, non uno zero finto. Segue il tema: nel tema chiaro HUD, pulsanti della mappa e banner di manovra sono chiari, leggibili al sole.
...
- Il file da importare può pesare al massimo **50 MB** (un archivio nazionale è intorno a 1,5 MB): oltre, l'app lo rifiuta prima di leggerlo invece di rischiare di bloccarsi. Il DB importato è compreso nel backup dello storico.
...
- **Formato compatto**: le righe si salvano in colonne (array tipizzati, `js/rows-codec.js`) e non come un oggetto per campione, e occupano circa 3 volte meno spazio (un'ora di log: 31,8 → 9,6 MB prima della compressione di IndexedDB). Tempo, posizione e velocità sono esatti; i sensori sono in Float32, quindi nel CSV circa una cella su 5000 cambia di 1 sull'ultima cifra. I giri salvati prima vengono convertiti in sottofondo, uno alla volta, qualche secondo dopo l'avvio e mai durante un log. Il formato del backup non cambia. In lettura ogni riga torna un oggetto con una sagoma fissa: costruita chiave per chiave pesava in memoria circa 3 volte una riga registrata (~2 KB contro ~660 B), e lo Stop di un giro di 3 h, che rilegge dal disco tutte le righe, costava ~420 MB in più invece di ~140 (misurato in Node).
...
- Tap → dettaglio: statistiche (con media e tempo in marcia e soste, calcolati come nella card PNG: si esclude la velocità sotto 5 km/h, nei buchi o congelata in galleria), **replay della traccia su mappa**, **grafici di velocità e piega di tutto il giro** (stessa `drawChart` dei grafici dal vivo, ma su tutte le righe invece che sugli ultimi 60 s), e **Export CSV / GPX / video / Card PNG / Elimina**.
- **Backup e ripristino** (`💾 Backup giri` / `♻ Ripristina giri`): un file JSON con tutto lo storico, righe e tracce comprese, che si può rimettere dentro l'app. Serve perché i giri vivono in IndexedDB e "cancella dati del sito", un browser che sfratta lo spazio o un telefono nuovo li perdono — mentre CSV e GPX sono per singolo giro e non si reimportano. Il ripristino aggiunge solo i giri che mancano (riconosciuti da inizio + durata, non dall'id), salta le voci malformate e non sovrascrive mai una sessione esistente. Nel file va anche il **DB autovelox importato**, se c'è: al ripristino si carica da solo se l'app non ne ha uno, altrimenti chiede prima di sostituire quello attuale. I backup delle versioni precedenti restano validi.
...
- **Tile della mappa 3D**: nel percorso offline ogni frame aspetta che la mappa abbia tutte le tile della vista (satellite, rilievo, strade) prima di essere catturato, e il primo frame aspetta fino a 20 s la vista di partenza. Prima il frame si catturava subito e a 12× il satellite restava sgranato o mancante. Su rete lenta l'export si allunga e la riga di stato dice «scarico tile mappa…»; una richiesta appesa si aspetta al massimo 4 s, e dopo tre di fila l'attesa scende a 0,3 s finché le tile tornano ad arrivare. Il render realtime (browser senza WebCodecs) non può fermarsi ad aspettare: lì le tile possono ancora arrivare in ritardo.
- **Slow-mo curve** (spento di default): dove la piega supera 25° o la vibrazione 0,5 g per almeno 1,5 s il video rallenta a 0,35×, qualunque velocità sia scelta. Prima era sempre acceso: a 12× il video alternava tratti veloci a tratti più lenti del tempo reale, e su un giro con molte curve la maggior parte del video finiva in slow-mo. Spento, la velocità resta costante per tutto il giro.
...
| curva 30° + vibrazione ±0,3 g (0,17 g RMS) | 30,9° | **1,0°** |
| curva 30° + vibrazione ±0,6 g (0,35 g RMS) | 25,8° | **2,0°** |
| curva 30° + vibrazione 0,3 g RMS | — | **1,6°** |
| curva 30° + vibrazione 0,6 g RMS | — | **4,2°** |
...
(`tests/gyro-bias-still.test.mjs`): senza nessuna sosta l'errore resta ~5° anche in rettilineo
e ~6° in curva; dopo 10 s da fermi scende a 0,6° in rettilineo (1,7° con 0,3 g RMS di
vibrazione del minimo) e in curva a 0,6°, uguale a quello senza bias. Una versione precedente
...
Altri casi (`tests/lean-dynamics.test.mjs`): GPS assente per 8 s a metà curva, sotto il grado;
segno del giroscopio invertito all'avvio, corretto in ~1,5 s nei primi metri lenti e poi stesso
risultato; frenata da 5 m/s² tenendo 30° di piega, fino a ~6,5° durante la manovra. **Subito
dopo la frenata** però la stima sbaglia di 7-9° per circa un secondo (la velocità fusa resta
indietro rispetto a quella vera e la compensazione centripeta esagera) e torna sotto il grado
in 3-5 s: una versione precedente di questo testo diceva "rientro a 0,0° in un secondo".
...
> 0,3/√3 ≈ 0,17 g RMS; le righe "g RMS" sono misurate a quel valore efficace. Le righe con
> vibrazione della colonna "adesso" sono medie su 5 semi, 10 s di misura dopo 5 s di
> assestamento, con il GPS che riancora la velocità ogni secondo come nell'app
> (`tests/lean-sim.mjs`; `tests/vib-adapt.test.mjs` le ricontrolla a ogni push). Una revisione
> precedente di queste righe non riancorava la velocità al GPS: la velocità fusa derivava di
> ±2,5 m/s integrando il rumore e gli errori venivano circa il doppio (per esempio 3,5° invece
> di 1,6° a 0,3 g RMS).
...
rimisurate come sopra (media su 5 semi, 10 s dopo 5 s, GPS a 1 Hz); "prima" = adattamento spento.

| scenario | prima (filtro fisso) | adesso (adattivo) |
|---|---|---|
| curva tenuta 30°, quieta | 0,0° | **0,0°** |
| curva tenuta 30° + vibrazione ±0,3 g, gyro ±5 °/s | 1,3° | **1,0°** |
| curva tenuta 30° + vibrazione ±0,6 g, gyro ±5 °/s | 2,3° | **2,0°** |
| curva tenuta 30° + vibrazione ±0,6 g, gyro ±20 °/s | 3,1° | **2,3°** |
| curva tenuta 40° + vibrazione ±0,6 g, gyro ±5 °/s | 2,6° | **1,8°** |
| curva tenuta 30° + vibrazione 0,6 g RMS, gyro ±5 °/s | 3,8° | **4,2°** |
...
La lettura: in curva tenuta l'adattamento toglie fra un quarto e un terzo dell'errore a
vibrazione moderata (1,3° → 1,0°, 2,6° → 1,8° a 40°), anche col rumore fortissimo sul
giroscopio (3,1° → 2,3°). A vibrazione fortissima, 0,6 g RMS veri, **peggiora**: 4,2° contro
3,8° del filtro fisso. Nella chicane veloce la dinamica paga ~1° per l'adattamento.
